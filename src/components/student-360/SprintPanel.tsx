...
export function SprintPanel({ items }: SprintPanelProps) {
  ...
  return (
    <section className="overflow-hidden rounded border border-outline-variant bg-surface">
      <PanelHeader title="Sprints" subtitle="Current direction" count={items.length} />

      {items.length > 0 ? (
        <div className="divide-y divide-[color:var(--outline-variant)]">
          {active.map((item) => (
            <div key={item.id} className="bg-surface-high/40 px-5 py-4">
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <Icon name="play_circle" filled className="text-[16px] text-on-surface" />
          ...
          {rest.map((item) => (
            <div key={item.id} className={cn("px-5 py-4", active.length > 0 && "opacity-70")}>
              <div className="flex items-start justify-between gap-4">
                <p className="font-medium text-on-surface">{item.title}</p>
                {item.status && (
                  <span className="shrink-0 rounded border border-outline-variant bg-surface-high px-2 py-1 text-[10px] font-mono font-semibold text-on-surface">
